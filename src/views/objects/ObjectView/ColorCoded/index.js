...
const ColorCoded = ({ contents }) => {
  ...
  const subKeyed = contents[0].object.contents;

  // Define the JSON object.
  const meta = subKeyed.meta;
  const descriptionDomain = subKeyed.descriptionDomain;
  const provenanceDomain = subKeyed.provenanceDomain;
  const usabilityDomain = subKeyed.usabilityDomain;
  const executionDomain = subKeyed.executionDomain;
  const errorDomain = subKeyed.errorDomain;
  const ioDomain = subKeyed.ioDomain;
  const parametricDomain = subKeyed.parametricDomain;
